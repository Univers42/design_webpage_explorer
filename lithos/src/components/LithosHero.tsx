...
/** Radius (px) of the cursor spotlight that reveals the second image. */
...
/** Reveal image — shown only through the spotlight mask. */
const BG_IMAGE_2 =
...
/**
 * Paints a soft radial "spotlight" onto a hidden canvas, exports it as a data
 * URL, and applies it as an alpha mask over a full-bleed copy of the reveal
 * image. The second image therefore only shows through the glowing circle that
 * trails the cursor; everywhere else the mask is transparent and the base image
 * below shows through.
 */
function RevealLayer({ image, cursorX, cursorY }: RevealLayerProps) {
  ...
  useLayoutEffect(() => {
    ...
    return () => window.removeEventListener('resize', sizeToViewport)
  }, [])

  // Re-paint the spotlight mask whenever the (smoothed) cursor moves. Runs in a
  // layout effect so the mask is committed before the browser paints the frame,
  // which keeps the reveal image from ever flashing fully on top of the base.
  useLayoutEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          // Hidden until the first mask is painted so the reveal image never
          // momentarily covers the base image on mount.
          opacity: maskUrl ? 1 : 0,
...
export default function LithosHero() {
  const mouse = useRef({ x: -999, y: -999 }) // raw pointer position
  const smooth = useRef({ x: -999, y: -999 }) // eased position fed to the mask
  const rafRef = useRef(0)
  const [cursorPos, setCursorPos] = useState({ x: -999, y: -999 })

  // Track the raw pointer and ease it toward the target every animation frame,
  // so the spotlight trails the cursor with a smooth, weighted motion.
  useEffect(() => {
  ...
  return (
    <>
      {/* ---------- Navigation (fixed over the hero) ---------- */}
      ...
      <section
        ...
        style={{ height: '100dvh' }}
      >
        {/* 1. Base image (z-10) with a slow Ken Burns zoom-out */}
        <div
          className="absolute inset-0 bg-center bg-cover bg-no-repeat z-10 hero-zoom"
          style={{ backgroundImage: `url("${BG_IMAGE_1}")` }}
        />

        {/* 2. Cursor spotlight reveal layer (z-30) */}
